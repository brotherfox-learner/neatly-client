...
const SKELETON_DATA = [
  { day: 'Sun', percent: 45 }, { day: 'Mon', percent: 40 },
  { day: 'Tue', percent: 55 }, { day: 'Wed', percent: 35 },
  { day: 'Thu', percent: 60 }, { day: 'Fri', percent: 80 },
  { day: 'Sat', percent: 90 },
]
...
const chartOption = computed(() => ({
  ...
  xAxis: {
    ...
    axisLine: { show: false },
    axisTick: { show: false },
    axisLabel: { color: 'var(--gray-700)', fontSize: 12 },
  },
  yAxis: {
    type: 'value',
    min: 0, max: 100,
  ...
  series: [{
    ...
    itemStyle: {
      color: props.loading ? 'var(--gray-200)' : 'var(--orange-500)',
...
<template>
  <article class="flex flex-col gap-[24px]">
    ...
    <div class="w-full min-h-[200px]">
      <div v-if="!loading && !hasData" class="flex items-center justify-center h-[235px] body-2 text-gray-400">
        No data for selected range
      </div>
      <VChart v-else class="w-full h-[235px]" :option="chartOption" autoresize />
    </div>
  </article>
</template>